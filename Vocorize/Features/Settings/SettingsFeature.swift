    ...
    Reduce { state, action in
      ...
      case let .keyEvent(keyEvent):
        ...
        if let key = keyEvent.key {
          state.$vocorizeSettings.withLock {
            $0.hotkey.key = key
      ...
      case let .toggleOpenOnLogin(enabled):
        state.$vocorizeSettings.withLock { $0.openOnLogin = enabled }
        return .run { _ in
          if enabled {
      ...
      case let .togglePreventSystemSleep(enabled):
        state.$vocorizeSettings.withLock { $0.preventSystemSleep = enabled }
        return .none

      case let .togglePauseMediaOnRecord(enabled):
        state.$vocorizeSettings.withLock { $0.pauseMediaOnRecord = enabled }
        return .none
      ...
      case let .modelDownload(.selectModel(newModel)):
        // Also store it in vocorizeSettings:
        state.$vocorizeSettings.withLock {
      ...
      case let .toggleSaveTranscriptionHistory(enabled):
        state.$vocorizeSettings.withLock { $0.saveTranscriptionHistory = enabled }
