  ...
  func startHotKeyMonitoringEffect() -> Effect<Action> {
    .run { send in
      ...
      keyEventMonitor.handleKeyEvent { keyEvent in
        ...
        hotKeyProcessor.hotkey = vocorizeSettings.hotkey
        hotKeyProcessor.useDoubleTapOnly = vocorizeSettings.useDoubleTapOnly
        ...
        case .startRecording:
          ...
          return vocorizeSettings.useDoubleTapOnly || keyEvent.key != nil
...
private extension TranscriptionFeature {
  func handleStartRecording(_ state: inout State) -> Effect<Action> {
    ...
    if state.vocorizeSettings.preventSystemSleep {
  ...
  func handleStopRecording(_ state: inout State) -> Effect<Action> {
    ...
    let durationIsLongEnough: Bool = {
      ...
      return Date().timeIntervalSince(startTime) > state.vocorizeSettings.minimumKeyTime
    }()
    ...
    state.error = nil
    let model = state.vocorizeSettings.selectedModel
    let language = state.vocorizeSettings.outputLanguage
  ...
  func finalizeRecordingAndStoreTranscript(
    ...
    .run { send in
      do {
        ...
        if vocorizeSettings.saveTranscriptionHistory {
          ...
          let supportDir = try fm.url(
            ...
            create: true
          			...
          			let ourAppFolder = supportDir.appendingPathComponent("com.tanvir.Vocorize", isDirectory: true)
          ...
          transcriptionHistory.withLock { history in
            ...
            if let maxEntries = vocorizeSettings.maxHistoryEntries, maxEntries > 0 {
...
private extension TranscriptionFeature {
  func preventSystemSleep(_ state: inout State) {
    // Prevent system sleep during recording
    		let reasonForActivity = "Vocorize Voice Recording" as CFString
